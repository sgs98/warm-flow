...
- `warm-flow-orm`：ORM 适配层，实现 core 的 `WarmDao` 与实体。按「ORM × 框架」矩阵展开：
  - `warm-flow-mybatis`：`*-core` + `*-sb3-starter`(SpringBoot3) + `*-sb4-starter`(SpringBoot4)。
  - `warm-flow-mybatis-plus`：同上三件套。
- `warm-flow-plugin`：可插拔扩展。
  - `warm-flow-plugin-modes`：Spring 框架模式与 SpEL 表达式实现（`*-sb`）。
...
- `warm-flow-core` **禁止**出现 `org.springframework.*`、`com.baomidou.*` 等具体框架 / ORM 依赖与 import（当前 core 已做到零此类依赖，必须保持）。
- 框架差异通过 `FrameInvoker`、SPI、各 starter 解决；ORM 差异通过 `WarmDao` 实现与各 orm 模块解决。

### 4. 多生态对齐

- 改某个 starter 的能力时，确认是否需要在 `sb3` / `sb4` 以及对应 ORM 模块同步；不要只改一处导致生态不一致。
...
- 表达式（条件 / 办理人 / 票签）与监听器是引擎对外扩展点，新增策略沿用 `condition` / `strategy` / `listener` 既有接口与 `plugin-modes` 的 SpEL 实现模式。
...
- **新增 ORM 支持**：在 `warm-flow-orm` 下按现有三件套（`core` + `sb3`/`sb4` starter）建模块，实现 `WarmDao`、实体与 ID 生成适配。
- **新增框架适配**：提供注入 `FrameInvoker` 的启动逻辑与对应 Spring 自动装配文件（`META-INF/spring/...AutoConfiguration.imports`）。
...
- 变更 JDK 基线（升 / 降均属基线调整，需用户明确批准）、删除编译 / 打包插件、放宽开源协议或弱化品牌信息。
