...
- 画布底浅灰 `#eef1f6`（`--wf-bg-page`），白卡片 / 白胶囊才能立住；不要画布、顶栏、节点全白。
- 审批卡片（中间节点）：固定 **260 × 76**、14px 圆角、白底 + 淡蓝边 `rgba(64,158,255,.18)` + 轻阴影；左侧 36 蓝渐变图标 + 标题 / 办理人首字头像，右侧 › 用主色。无色条头。
- 开始 / 结束：同一套 **76 × 40** 胶囊（开始播放标、结束停止点）。加号白底蓝边蓝 +；网关 24 旋转圆角菱形 + 浅蓝底 `#ecf5ff`。设计态连线 `rgba(64,158,255,.55)`。
- **配色统一蓝 chroma，禁止橙色 `#FF8B00`/`#FFFCEB`**：网关菱形、边「+」加号、跳转条件徽标都用 `#409eff` / 浅蓝 `#ecf5ff`。仿钉钉网关**不要**调用 `applyClassicDesignColor`。
