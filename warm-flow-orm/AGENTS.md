...
> 本文件只写 `warm-flow-orm` 的差异化规则。通用工程与编码规范以仓库根 [`../AGENTS.md`](../AGENTS.md) 为准；规则优先级见根
`AGENTS.md`「规则优先级」。
...
- core 的 `WarmDao` / `WarmQuery` / `WarmServiceImpl` 与对应实体接口；改某 ORM 的 `WarmDaoImpl` 前，先看同 ORM 的 `*-core`
  实现与 starter 装配。
