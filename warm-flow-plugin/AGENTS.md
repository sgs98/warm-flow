...
> 本文件只写 `warm-flow-plugin` 的差异化规则。通用工程与编码规范以仓库根 [`../AGENTS.md`](../AGENTS.md) 为准；规则优先级见根
`AGENTS.md`「规则优先级」。
...
- `warm-flow-plugin-modes`：Spring 框架模式与 SpEL 表达式实现（`ConditionStrategySpel`、`VoteSignStrategySpel`、
  `HandlerStrategySpel` 等，含 `SpelHelper` / 安全方法解析）。
- `warm-flow-plugin-json`：`JsonConvert` 的 SPI 实现。`*-json-v1`（`JsonConvertSnack`/`Snack4`/`Jackson`/`FastJson`/
  `Gson`）；`*-json-jackson3`。包根 `org.dromara.warm.plugin.json`。
- `warm-flow-plugin-ui`：设计器 / 流程图后端。`*-ui-core`（`service` / `dto` / `vo`）、`*-ui-sb-web`（Spring controller）、
  `*-vue3-ui`（打包前端资源）。包根 `org.dromara.warm.flow.ui`。
...
涉及 SPI / 表达式 / UI 契约时，确认注册文件与对应 Spring 实现同步。
