...
- `warm-flow-orm`：ORM 适配，按「ORM × (sb/sb3/sb4)」矩阵实现 `WarmDao`。
...
8. **扩展机制**：新增 JSON 走 `JsonConvert` + `META-INF/services`；新增 ORM / 框架走五件套矩阵 + 对应 Spring 自动装配（`spring.factories` / `AutoConfiguration.imports`）。
