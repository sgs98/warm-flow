...
> 主入口 **UI 库无关**，不内置 UI 组件库。渲染 `FlowDesigner` 前必须先 `setUiAdapter(elementPlusAdapter)`，否则设计器内的中性组件 `wf-*` 无法映射到 UI 组件、画布不渲染。
...
内置 `element-plus` 适配器覆盖当前支持场景。若要接入其它 UI 库，实现 `UiAdapter` 契约并 `setUiAdapter(yourAdapter)` 即可——核心逻辑与 UI 库解耦，无需改库源码。
...
**最佳实践**
- **脱上下文调用**：`message` / `notify` / `alert` 等由设计器内部命令式触发，需使用 Element Plus 的全局 API，不依赖组件树实例。
...
- **参考实现**：`src/ui/elementPlusAdapter.ts`（含 28 个组件映射、指令实现和命令式反馈）。
...
宿主需提供：`vue`、`vue-router`、`pinia`、`@logicflow/core`、`@logicflow/extension` 和 `element-plus`；通过 `@dromara/warm-flow-designer/element-plus` 引入适配器并调用 `setUiAdapter(...)`。图标已内置（iconify 离线集 `ep` + `wf`，随库打包，无需额外依赖）。
